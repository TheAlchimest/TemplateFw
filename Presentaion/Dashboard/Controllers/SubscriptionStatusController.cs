...
using Urls = Dashboard.Common.WebClientHelpers.InternalApiDictionary.SubscriptionStatusUrls;

namespace TemplateFw.Dashboard.Controllers
{
    //[Authorize(Roles = RoleProvider.SubscriptionStatus)]
    public class SubscriptionStatusController : WebBaseController<SubscriptionStatusController>
    {
        private readonly RequestUrlHelper _api = ApiRequestHelper.InternalAPI;
        private readonly SubscriptionStatusDtoInsertValidator _insertValidator;
        private readonly SubscriptionStatusDtoUpdateValidator _updateValidator;

        public  SubscriptionStatusController(SubscriptionStatusDtoInsertValidator validator, SubscriptionStatusDtoUpdateValidator updateValidator)
        {
    ...
    [HttpGet]
        public async Task<IActionResult> Add()
        {
            try
            {
                ViewBag.ActionUrl = "/subscriptionStatus/create";
                return View("Save");
    }
        ...
        public async Task<IActionResult> Edit(string id)
        {
            try
            {
                ...
                var apiResult = await _api.GetAsync<GenericApiResponse<SubscriptionStatusDto>>(url);
                ViewBag.ActionUrl = "/subscriptionStatus/update";
        ...
        #region Create
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] SubscriptionStatusDto dto)
        {
            try
            {
        ...
        #region Update
        [HttpPost]
        public async Task<IActionResult> Update([FromBody] SubscriptionStatusDto dto)
        {
            try
            {
        ...
        {
            SubscriptionStatusFilter filter = new SubscriptionStatusFilter();
            var apiResult = await _api.PostAsync<GenericApiResponse<PagedList<SubscriptionStatusInfoDto>>>(Urls.GetPaged, filter);
        ...
        [HttpGet]
        public async Task<IActionResult> IndexContent([FromQuery] SubscriptionStatusFilter filter)
        {
            var apiResult = await _api.PostAsync<GenericApiResponse<PagedList<SubscriptionStatusInfoDto>>>(Urls.GetPaged, filter);
            return ReturnViewResponse(apiResult, OperationTypes.GetContent);
        }
