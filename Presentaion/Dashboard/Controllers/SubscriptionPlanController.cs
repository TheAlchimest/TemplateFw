...
using Urls = Dashboard.Common.WebClientHelpers.InternalApiDictionary.SubscriptionPlanUrls;

namespace TemplateFw.Dashboard.Controllers
{
    //[Authorize(Roles = RoleProvider.SubscriptionPlan)]
    public class SubscriptionPlanController : WebBaseController<SubscriptionPlanController>
    {
        private readonly RequestUrlHelper _api = ApiRequestHelper.InternalAPI;
        private readonly SubscriptionPlanDtoInsertValidator _insertValidator;
        private readonly SubscriptionPlanDtoUpdateValidator _updateValidator;

        public  SubscriptionPlanController(SubscriptionPlanDtoInsertValidator validator, SubscriptionPlanDtoUpdateValidator updateValidator)
        {
    ...
    [HttpGet]
        public async Task<IActionResult> Add()
        {
            try
            {
                ViewBag.ActionUrl = "/subscriptionPlan/create";
                return View("Save");
    }
        ...
        public async Task<IActionResult> Edit(string id)
        {
            try
            {
                ...
                var apiResult = await _api.GetAsync<GenericApiResponse<SubscriptionPlanDto>>(url);
                ViewBag.ActionUrl = "/subscriptionPlan/update";
        ...
        #region Update
        [HttpPost]
        public async Task<IActionResult> Update([FromBody] SubscriptionPlanDto dto)
        {
            try
            {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var apiResult = await _api.PostAsync<GenericApiResponse<PagedList<SubscriptionPlanInfoDto>>>(Urls.GetPaged, filter);
        ...
        [HttpGet]
        public async Task<IActionResult> IndexContent([FromQuery] SubscriptionPlanFilter filter)
        {
            var apiResult = await _api.PostAsync<GenericApiResponse<PagedList<SubscriptionPlanInfoDto>>>(Urls.GetPaged, filter);
            return ReturnViewResponse(apiResult, OperationTypes.GetContent);
        }
