...
{
    //[Authorize(Roles = RoleProvider.Category)]
    public class CategoryController : WebBaseController<CategoryController>
    {
        private readonly RequestUrlHelper _api = ApiRequestHelper.InternalAPI;
        private readonly CategoryDtoInsertValidator _insertValidator;
        private readonly CategoryDtoUpdateValidator _updateValidator;

        public  CategoryController(CategoryDtoInsertValidator validator, CategoryDtoUpdateValidator updateValidator)
        {
    ...
    [HttpGet]
        public async Task<IActionResult> Add()
        {
            try
            {
                ViewBag.ActionUrl = "/category/create";
                return View("Save");
    }
        ...
        public async Task<IActionResult> Edit(string id)
        {
            try
            {
                ...
                var apiResult = await _api.GetAsync<GenericApiResponse<CategoryDto>>(url);
                ViewBag.ActionUrl = "/category/update";
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var apiResult = await _api.PostAsync<GenericApiResponse<PagedList<CategoryInfoDto>>>(Urls.GetPaged, filter);
        ...
        [HttpGet]
        public async Task<IActionResult> IndexContent([FromQuery] CategoryFilter filter)
        {
            var apiResult = await _api.PostAsync<GenericApiResponse<PagedList<CategoryInfoDto>>>(Urls.GetPaged, filter);
            return ReturnViewResponse(apiResult, OperationTypes.GetContent);
        }
