...
using Urls = Dashboard.Common.WebClientHelpers.InternalApiDictionary.PaymentUrls;

namespace TemplateFw.Dashboard.Controllers
{
    //[Authorize(Roles = RoleProvider.Payment)]
    public class PaymentController : WebBaseController<PaymentController>
    {
        private readonly RequestUrlHelper _api = ApiRequestHelper.InternalAPI;
        private readonly PaymentDtoInsertValidator _insertValidator;
        private readonly PaymentDtoUpdateValidator _updateValidator;

        public  PaymentController(PaymentDtoInsertValidator validator, PaymentDtoUpdateValidator updateValidator)
        {
    ...
    [HttpGet]
        public async Task<IActionResult> Add()
        {
            try
            {
                ViewBag.ActionUrl = "/payment/create";
                return View("Save");
    }
        ...
        public async Task<IActionResult> Edit(string id)
        {
            try
            {
                ...
                var apiResult = await _api.GetAsync<GenericApiResponse<PaymentDto>>(url);
                ViewBag.ActionUrl = "/payment/update";
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var apiResult = await _api.PostAsync<GenericApiResponse<PagedList<PaymentInfoDto>>>(Urls.GetPaged, filter);
        ...
        [HttpGet]
        public async Task<IActionResult> IndexContent([FromQuery] PaymentFilter filter)
        {
            var apiResult = await _api.PostAsync<GenericApiResponse<PagedList<PaymentInfoDto>>>(Urls.GetPaged, filter);
            return ReturnViewResponse(apiResult, OperationTypes.GetContent);
        }
        ...
        [HttpGet]
        public async Task<JsonResult> Lookup(int? subscriptionId = null)
        {
            string url = string.Format(Urls.GetLookup, subscriptionId);
