...
namespace TemplateFw.Application.Services
{
    public class ServiceTypeService : BaseService, IServiceTypeService
    {
        ...
        #region Constructor
        public ServiceTypeService(IServiceTypeRepository repository, IMapper mapper, IUserInfoService userInfoService) : base(mapper, userInfoService)
        {
        ...
        public async Task<ServiceTypeInfoDto> GetInfoByIdAsync(int id, EnumLanguage lang)
        {
        ...
        {
            var filter = new ServiceTypeFilter
            {
                
            };
            return await GetAllAsLookupAsync(filter);
        }
        public async Task<List<LookupDto>> GetAllAsLookupAsync(ServiceTypeFilter filter)
        {
            var list = await GetAllAsync(filter);
            return list.Select(e => new LookupDto(e.ServiceTypeId,e.Name)).ToList();
        }
        #endregion
    }
}
