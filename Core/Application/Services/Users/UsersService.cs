...
namespace TemplateFw.Application.Services
{
    public class UsersService : BaseService, IUsersService
    {
        ...
        #region Constructor
        public UsersService(IUsersRepository repository, IMapper mapper, IUserInfoService userInfoService) : base(mapper, userInfoService)
        {
        ...
        public async Task<UsersInfoDto> GetInfoByIdAsync(int id, EnumLanguage lang)
        {
        ...
        public async Task<List<LookupDto>> GetAllAsLookupAsync(UsersFilter filter)
        {
            var list = await GetAllAsync(filter);
            return list.Select(e => new LookupDto(e.UserId,e.FirstName)).ToList();
        }
        #endregion
    }
}
