...
namespace TemplateFw.Application.Services
{
    public class ContactUsService : BaseService, IContactUsService
    {
        ...
        #region Constructor
        public ContactUsService(IContactUsRepository repository, IMapper mapper, IUserInfoService userInfoService) : base(mapper, userInfoService)
        {
        ...
        public async Task<ContactUsInfoDto> GetInfoByIdAsync(int id, EnumLanguage lang)
        {
        ...
        {
            var filter = new ContactUsFilter
            {
                
            };
            return await GetAllAsLookupAsync(filter);
        }
        public async Task<List<LookupDto>> GetAllAsLookupAsync(ContactUsFilter filter)
        {
            var list = await GetAllAsync(filter);
            return list.Select(e => new LookupDto(e.MessageId,e.Name)).ToList();
        }
        #endregion
    }
}
